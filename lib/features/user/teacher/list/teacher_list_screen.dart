...
class _TeacherListScreenState extends State<TeacherListScreen> {
  ...
  Map<String, List<String>> routineDocIds =
      {}; // Stores document IDs (batch_day) for each routine
  ...
  void _initStreams() {
    _teacherSubscription =
        _teacherService.streamAllTeachers().listen((teachers) {
      if (mounted) {
        setState(() {
    ...
    _routineSubscription =
        _routineService.streamAllRoutines().listen((allRoutines) {
      if (mounted) {
  ...
  void _processRoutines(List<Routine> allRoutines) {
    ...
    for (final routine in allRoutines) {
      for (final routineClass in routine.classes) {
        final teacherInitials =
            routineClass.teacherInitial.trim().toUpperCase();
        ...
        if (teacherInitials.isNotEmpty) {
          ...
          docIdsMap
              .putIfAbsent(key, () => [])
              .add(routine.id); // Store document ID (batch_day)
        }
      }
    }
  ...
  Future<void> _generateTeacherPDF(Teacher teacher) async {
    try {
      ...
      for (final routine in allRoutines) {
        final teacherClasses = routine.classes
            .where(
                (c) => c.teacherInitial.trim().toUpperCase() == teacherInitials)
            .toList();
        if (teacherClasses.isNotEmpty) {
          teacherRoutines.add(Routine(
            id: routine.id,
            batch: routine.batch.isEmpty && routine.id.contains('_')
      ...
      if (teacherRoutines.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('No classes scheduled for this teacher.')),
          );
        }
        return;
      }
  ...
  Widget _buildTeacherActionButtons(Teacher teacher, int index,
      bool routineVisible, Color roleCategoryColor) {
  ...
  Widget build(BuildContext context) {
    ...
    sortedTeachers.sort(
        (a, b) => _getRolePriority(a.role).compareTo(_getRolePriority(b.role)));
              ...
              : sortedTeachers.isEmpty
                  ? Center(
                      child: Text(
                        ...
                        style: TextStyle(fontSize: 18, color: isDark ? ITMColors.darkTextSecondary : Colors.black54),
                      ),
                    )
                            ...
                            child: Center(
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  boxShadow: [
                                    BoxShadow(
                                      color: Colors.teal.withOpacity(isDark ? 0.3 : 0.2),
                                      blurRadius: 16,
                            ...
                            child: Container(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final roleCategory =
                                      _getRoleCategory(teacher.role);
                                  final roleCategoryColor =
                                      _getRoleCategoryColor(teacher.role);
                                                            ...
                                                            Text(
                                                              ...
                                                              style:
                                                                ...
                                                                fontWeight:
                                                                    FontWeight
                                                                        .bold,
                                                                color: isDark ? ITMColors.darkTextPrimary : Colors
                                                                    .black87,
                                                              ),
                                                              maxLines: 2,
                                                              overflow:
...
class TeacherRoutineDetailsSheet extends StatefulWidget {
  ...
  @override
  State<TeacherRoutineDetailsSheet> createState() =>
      _TeacherRoutineDetailsSheetState();
}

class _TeacherRoutineDetailsSheetState
    extends State<TeacherRoutineDetailsSheet> {
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isSelected ? widget.roleCategoryColor : (isDark ? ITMColors.darkSurface : Colors.grey[200]),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(
                day.substring(0, 3),
                style: TextStyle(
                  color: isSelected ? Colors.white : (isDark ? ITMColors.darkTextSecondary : Colors.black87),
  ...
  Widget _buildRoutineList() {
    ...
    final consultingForDay = widget.teacher.consultingHours
        .where((slot) => slot.startsWith(dayShort))
        .toList();
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                          color: isDark ? ITMColors.darkTextSecondary : Colors.black87),
                    ),
                  ],
                ),
              )),
                    ...
                    Row(
                      children: [
                        Icon(Icons.class_rounded,
                            color: widget.roleCategoryColor, size: 18),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              ...
                              Text(
                                c.courseCode,
                                style: TextStyle(
                                    fontSize: 12, color: isDark ? ITMColors.darkTextTertiary : Colors.grey[600]),
                              ),
                            ],
                          ),
                        ),
