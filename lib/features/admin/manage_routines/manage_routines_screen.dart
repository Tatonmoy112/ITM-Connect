...
class _ManageRoutineScreenState extends State<ManageRoutineScreen>
  ...
  Future<String?> _checkGlobalTimeConflict(
      String teacherInitial, String timeRange, String day,
                          ...
                          children: [
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                errorText:
                                    showCourseNameError ? 'Required' : null,
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                contentPadding: const EdgeInsets.symmetric(
                                    horizontal: 12, vertical: 10),
                                errorText:
                                    showCourseCodeError ? 'Required' : null,
                                border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(8)),
                              ),
                            ),
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                contentPadding: const EdgeInsets.symmetric(
                                    horizontal: 12, vertical: 10),
                                errorText:
                                    showTeacherInitialError ? 'Required' : null,
                                border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(8)),
                              ),
                            ),
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: DropdownButton<String>(
                                      value: RoutineService.classSlots
                                              .contains(time.text)
                                          ? time.text
                                          : null,
                                      ...
                                      items:
                                          RoutineService.classSlots.map((slot) {
                        ...
                        ElevatedButton(
                          ...
                          onPressed: isLoading
                              ...
                              : () async {
                                  ...
                                  final tInitial =
                                      teacherInitial.text.trim().toUpperCase();
                                  ...
                                  if (showCourseNameError ||
                                    ...
                                    if (showCourseCodeError)
                                      missingFields.add('Course Code');
                                    if (showTeacherError)
                                      missingFields.add('Teacher Name');
                                    if (showTeacherInitialError)
                                      missingFields.add('Teacher Initial');
                                    if (showRoomError)
                                  ...
                                  if (!_isValidInput(cName) ||
                                      !_isValidInput(cCode) ||
                                      !_isValidInput(tName) ||
                                      !_isValidInput(tInitial) ||
                                      !_isValidInput(rRoom)) {
                                    if (mounted) {
                                      showDialog(
                                        context: context,
                                        builder: (context) => AlertDialog(
                                          ...
                                          content: const Text(
                                              'Input contains invalid characters or SQL keywords. Please use only letters, numbers, spaces, hyphens, underscores, and dots.'),
                                          actions: [
                                            ElevatedButton(
                                              style: ElevatedButton.styleFrom(
                                  ...
                                  try {
                                    ...
                                    final existingRoutine =
                                        await _routineService.getRoutine(docId);
                                    final existingClasses =
                                        existingRoutine?.classes ?? [];

                                    // Parse the new class time
                                    final newTimeRange =
                                        _parseTimeRange(newClass.time);
                                    ...
                                    for (int i = 0;
                                      ...
                                      if (routine == null || i != index) {
                                        final existingTimeRange =
                                            _parseTimeRange(existingClass.time);

                                        if (existingTimeRange != null &&
                                            _timesOverlap(newTimeRange,
                                                existingTimeRange)) {
                                          if (mounted) {
                                            showDialog(
                                              context: context,
                                    ...
                                    for (int i = 0;
                                      ...
                                      if (existingClass.teacherInitial ==
                                    ...
                                    final globalConflictMsg =
                                        await _checkGlobalTimeConflict(
                                      newClass.teacherInitial,
                                      newClass.time,
                                      selectedDay, // Pass the day to only check conflicts on the same day
                                      excludeDocId:
                                          routine == null ? null : docId,
                                    );
                                    ...
                                    if (globalConflictMsg != null) {
                                      if (mounted) {
                                        showDialog(
                                          ...
                                          builder: (context) => AlertDialog(
                                            title: const Text(
                                                'Time Conflict - Cannot Add Class'),
                                    ...
                                    if (routine == null) {
                                      await _routineService.addClass(
                                          docId, newClass);
                                    } else {
                                      // update by index
                                      await _routineService.updateClass(
                                          docId, index!, newClass);
                                    }
  ...
  Future<BatchInfo?> _showBatchInfoDialog(String batchId,
      {BatchInfo? existingInfo}) async {
    final sessionController =
        TextEditingController(text: existingInfo?.session);
    final advisorController =
        TextEditingController(text: existingInfo?.advisorName);
    final studentsController =
        TextEditingController(text: existingInfo?.totalStudents);
    ...
    if (!RegExp(r'^\d+$').hasMatch(newBatch)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('Batch must contain only numbers (e.g., 11, not 1st)')),
        );
      }
      return;
    }
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  TextButton.icon(
                    onPressed: () async {
                      final updated = await _showBatchInfoDialog(selectedBatch,
                          existingInfo: info);
                      if (updated != null) {
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text('Batch info updated!')),
                          );
                        }
                      }
                    },
                    icon: Icon(
                        ...
                        size: 16),
                    label:
                        Text(isNotInitialized ? 'Add Details' : 'Edit Details'),
              ...
              Wrap(
                ...
                children: [
                  ...
                  _infoItem(
                      Icons.group_outlined, 'Students', info.totalStudents),
                ],
              ),
            ],
          ),
  ...
  void _deleteBatch(String batch) {
    ...
    _routineService.deleteBatch(batch).then((_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Batch removed from Firestore')));
      }
    }).catchError((e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _handleGoogleSheetSync() async {
    try {
      ...
      // Notify students about the routine update
  ...
  Future<void> _pickAndUploadCSV() async {
    try {
      ...
      if (rows[0].any((cell) =>
          ...
          cell.toString().toLowerCase().contains('course'))) {
        startRow = 1;
      }
      ...
      for (int i = startRow; i < rows.length; i++) {
        final row = rows[i];
        if (row.length < 8)
          continue; // Basic validation: batch, day, courseName, courseCode, teacherName, teacherInitial, room, time
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: DropdownButton<String>(
                                        isExpanded: true,
                                        underline: const SizedBox(),
                                        value: batchList.contains(selectedBatch)
                                            ? selectedBatch
                                            : null,
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (classes.isEmpty) {
                          return const Padding(
                            ...
                            child: Center(
                                child: Text(
                                    'No routine found for this day and batch.')),
                          );
                        }
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor:
            selectedBatch.isEmpty ? Colors.grey.shade400 : Colors.teal.shade600,
        foregroundColor: Colors.white,
