...
class TeacherService {
    ...
    return teachersCollection.snapshots().map((snapshot) {
      return snapshot.docs.map<Teacher>((doc) {
        ...
        String teacherInitial =
            (data['teacherInitial'] ?? doc.id).toString().trim();
        ...
        if (rawSchedule != null &&
          ...
          try {
            ...
            consultingHours = list
                .map((e) {
                  if (e != null && e is Map) {
                    final map = Map<String, dynamic>.from(e);
                    return map['fullString']?.toString() ??
                        "${map['day']} ${map['time']}";
                  }
                  return "";
                })
  ...
  Future<void> addOrUpdateTeacher({
    ...
    print(
        'Saving teacher $teacherInitial with ${consultingHours.length} slots: $consultingHours');
    ...
    await teachersCollection.doc(teacherInitial).set({
      ...
      'consultingHour': consultingHours.isNotEmpty
          ? consultingHours.join(', ')
